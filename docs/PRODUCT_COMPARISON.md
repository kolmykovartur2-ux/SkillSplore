# SkillSplore vs. profi.ru — structural comparison and roadmap
...
landing page), what SkillSplore now matches, and what's still a deliberate gap with a prioritized
...
## Where SkillSplore now matches this (changed today)

| profi.ru pattern | SkillSplore implementation |
| --- | --- |
...
| "Describe once, they quote you" value prop | Added an explicit 4-tile value-prop row on the homepage (`.props`) restating this exact flow, which SkillSplore already implements end-to-end (student requests → tutor responses with hidden competing rates). |
...
1. **Category landing pages** (`/subjects/piano`, `/subjects/programming`, etc.) — profi.ru gives
   ...
   SkillSplore currently routes that traffic to `/search?subjectId=…` instead of a dedicated page.
   *Priority: medium.* Would need a new route + a "from $X/hr" aggregate query per subject.

2. **Contact-from-card** — profi.ru's specialist cards have their own "Contact" button; SkillSplore's
...
3. **Passport/ID verification** — profi.ru explicitly verifies government ID, not just claimed
   qualifications. SkillSplore's tutor verification is qualification-document-based only (see
...
4. **Vertical-specific aggregate trust stats** ("98% positive review, 4.93 avg, 133,963 reviews in
   12 months") shown per category. SkillSplore shows trust indicators per *tutor* profile but not
   rolled up per category yet. *Priority: low* — needs volume to be meaningful; revisit once the
   marketplace has real review volume.

5. **Catalogue depth.** 97 subjects vs. profi.ru's ~1,928 in tutoring alone. SkillSplore's taxonomy is
...
model at scale. SkillSplore's data model and API (`TutoringRequest` → `RequestResponse`, hidden
