...
interface FeedItem { id: number; title: string; description: string; deliveryMode: string; city: string | null; country: string | null; subject: { name: string }; level: { name: string } | null; createdAt: string; student: PublicUser; myResponse: { id: number; status: string } | null }
interface Subject { id: number; name: string }

export function RequestFeed() {
  const { data: subs } = useApi<{ subjects: Subject[] }>('/taxonomy/subjects');
  const [filters, setFilters] = useState({ subjectId: '', mode: '', page: 1 });
